...
######################################################################
dir_lin = np.linspace(264, 276, 30)
...
for slope in slopes:
    for div in divs:
        ...
        filename = "K%sA%sT%sS%s_dir.nc" %(slope, div, period, std)
        data = os.path.join(path, filename)
        nc = Dataset(data, 'r')

        dp = nc.variables['dir'][seed_idx]
        x = (nc.variables['x'][:] - nc.variables['x'][0])*1e-3
        y = (nc.variables['y'][:] - nc.variables['y'][0])*1e-3

        ax = plt.subplot(4,4,i) 
        cs = ax.contourf(x, y, dp, dir_lin, extend='both', cmap=cm.PRGn_r)
...
cbar = plt.colorbar(cs,cax=cax, extend='both', orientation='horizontal',format = '%d', ticks=[264, 267, 270, 273, 276])
cbar.ax.xaxis.set_ticks_position('bottom')
cbar.ax.xaxis.set_label_position('bottom')
cbar.set_label(r'$\theta_w$  [$^\circ$]', fontsize=14, labelpad=14)
figname= "fig03.png"
